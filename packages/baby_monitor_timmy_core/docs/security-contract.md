...
This document describes the security-relevant backend and protocol surface that
is being extracted toward **`baby-monitor-timmy-core`**.

The repository is still private. The purpose of this document is to make the
current contract explicit while the code is being modularized.
...
2. move backend `functions/` and `firestore.rules` into the future core repo
